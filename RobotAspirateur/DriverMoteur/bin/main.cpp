    ...
    while (!stop)
	{
		...
		***************/
		...
		while (run)
		{
			...
			log->info("main : Attente d'un message");
			...
			else if (n != 0)
			{
				...
				if(regex_match(message, pattern))
				{
					...
					if (parametres[0] == "start")
					{
						...
						{
							log->info("main : demarrage des moteurs");
							b = controlleur.driver(stoi(parametres[1]), stoi(parametres[2]));
						}
						else
						{
							log->info("main : demarrage des moteurs avec " + parametres[3] + " pas");
							b = controlleur.driver(stoi(parametres[1]), stoi(parametres[2]), stoi(parametres[3]));
						}
						
						if (b != -1)
						{
							log->info("main :  succes de démarrage");
							retourClient="1\n";
						}
						else
						{
							log->info("main :  erreur de démarrage");
							retourClient="-1\n";
						}
					}
					else if (parametres[0] == "stop")
					{
						...
						if ((retour = controlleur.stopDriver()) != "-1")
						{
							log->info("main : succes de l'arret");
							retourClient=retour + "\n";
						}
						else
						{
					...
					{
						log->info("main : Rotation du robot");
				...
				log->info("main : retourClient =" + retourClient);
			...
			{
				log->erreur("main : Erreur message vide ou connexion interrompue");
				run = false;
			}
